...
func TestCannotSignUpWithAReplayAttack(t *testing.T) {
	...
	// First request
	resp, err := http.Post(postUrl, "application/json", strings.NewReader(webauthn.CreateMockPublicKeyCredential(clientData, supportedPublicKey, registrationData.RP.Id, true, true, false, false, credentialId, uuid.New().String())))
	if err != nil {
	...
	respReplay, err := http.Post(postUrl, "application/json", strings.NewReader(webauthn.CreateMockPublicKeyCredential(clientData, supportedPublicKey, webauthn.RP.Id, true, true, false, false, credentialId, uuid.New().String())))
	if err != nil {
...
func TestCannotSignUpIfClientDataIsIncorrect(t *testing.T) {
	for i := range 3 {
		...
		resp, err := http.Post(postUrl, "application/json", strings.NewReader(webauthn.CreateMockPublicKeyCredential(clientData, supportedPublicKey, webauthn.RP.Id, true, true, false, false, credentialId, uuid.New().String())))
...
func TestCannotSignUpIfPublicKeyCredentialIsIncorrect(t *testing.T) {
	...
	var incorrectPublicKeyCredentials []string = []string{
		webauthn.CreateMockPublicKeyCredential(clientDatas[0], supportedPublicKey, registrationDatas[0].RP.Id+"?", true, true, false, false, uuid.New().String(), uuid.New().String()), /*Test for incorrect relying party hash*/
		webauthn.CreateMockPublicKeyCredential(clientDatas[1], unsupportedPublicKey, registrationDatas[1].RP.Id, true, true, false, false, uuid.New().String(), uuid.New().String()),   /*test for unsupported public key algorithm*/
		webauthn.CreateMockPublicKeyCredential(clientDatas[2], supportedPublicKey, registrationDatas[2].RP.Id, false, true, false, false, uuid.New().String(), uuid.New().String()),    /* test for user present bit is false */
		webauthn.CreateMockPublicKeyCredential(clientDatas[3], supportedPublicKey, registrationDatas[3].RP.Id, true, false, false, false, uuid.New().String(), uuid.New().String()),    /* test for user verification bit is false */
		webauthn.CreateMockPublicKeyCredential(clientDatas[4], supportedPublicKey, registrationDatas[4].RP.Id, true, true, false, true, uuid.New().String(), uuid.New().String()),      /* test for backup state is set but backup eligibility is not set.*/
	}
...
func TestForValidBackupStateAndBackupElibility(t *testing.T) {
	...
	var publicKeyCredentials []string = []string{
		webauthn.CreateMockPublicKeyCredential(clientDatas[0], supportedPublicKey, registrationDatas[0].RP.Id, true, true, false, false, uuid.New().String(), uuid.New().String()), //backupEligibility = false & backupState = false
		webauthn.CreateMockPublicKeyCredential(clientDatas[1], supportedPublicKey, registrationDatas[1].RP.Id, true, true, true, false, uuid.New().String(), uuid.New().String()),  //backupEligibity = true & backupState = false
		webauthn.CreateMockPublicKeyCredential(clientDatas[2], supportedPublicKey, registrationDatas[2].RP.Id, true, true, true, true, uuid.New().String(), uuid.New().String()),   //backupEligibility = true & backupState = true
	}
